...
Dans le cadre de ce rapport, nous avons documenté une étude expérimentale visant à optimiser un modèle de classification d'images basé sur l’intelligence artificielle, préalablement fourni par l’enseignant.  
...
Il va falloir “jouer” avec les différents paramètres afin de rédiger un rapport sur nos observations des résultats obtenus. Nous devrons également essayer d’augmenter la précision finale du modèle.
...
- nos graphiques créés à la partie 3 ;
- l’augmentation du nombre d’échantillons par catégorie à **5000**.

Chaque test décrit dans ce document est associé à une branche dédiée, qui contient les modifications nécessaires à la réalisation de ce test, en plus d’autres branches non documentées dans ce rapport.